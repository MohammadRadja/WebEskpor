...
class PetakKebunController extends Controller
{
    ...
    {
        $petakan = PetakKebun::with(['kebun', 'tanaman'])
        ...
        try {
            ...
            if ($jumlah > $tanaman->stok_bibit) {
                ...
                return $request->ajax()
                    ...
                    : redirect()
                        ->back()
                        ->withErrors(['jumlah_tanaman' => $pesan])
                        ->withInput();
            }

            PetakKebun::create($request->all());

            return $request->ajax() ? response()->json(['message' => 'Petakan berhasil ditambahkan']) : redirect()->route('petak.kebun')->with('success', 'Petakan berhasil ditambahkan.');
        } catch (\Illuminate\Validation\ValidationException $e) {
            return $request->ajax() ? response()->json(['errors' => $e->errors()], 422) : redirect()->back()->withErrors($e->errors())->withInput();
        } catch (\Exception $e) {
            return $request->ajax() ? response()->json(['error' => 'Terjadi kesalahan server.'], 500) : redirect()->back()->with('error', 'Gagal menambahkan petakan.');
        }
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            return $request->ajax() ? response()->json(['message' => 'Petakan berhasil diperbarui']) : redirect()->route('petak.kebun')->with('success', 'Petakan berhasil diperbarui.');
        } catch (\Illuminate\Validation\ValidationException $e) {
            return $request->ajax() ? response()->json(['errors' => $e->errors()], 422) : redirect()->back()->withErrors($e->errors())->withInput();
        } catch (\Exception $e) {
            return $request->ajax() ? response()->json(['error' => 'Terjadi kesalahan server.'], 500) : redirect()->back()->with('error', 'Gagal memperbarui petakan.');
        }
    }
    ...
    public function destroy(Request $request, $id)
    {
        ...
        try {
            ...
            return $request->ajax() ? response()->json(['error' => 'Terjadi kesalahan saat menghapus.'], 500) : redirect()->back()->with('error', 'Gagal menghapus petakan.');
        }
    }
}
